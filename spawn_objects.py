...
class ObjectSpawner(Node):
    ...
    def spawn_box(self, name, x, y, z, size_x=0.5, size_y=0.5, size_z=0.5):
        """Spawn a box object"""
        request = SpawnEntity.Request()
        request.name = name
        request.xml = self.create_box_sdf(size_x, size_y, size_z)
    ...
    def spawn_cylinder(self, name, x, y, z, radius=0.3, height=0.5):
        """Spawn a cylinder object"""
        request = SpawnEntity.Request()
        request.name = name
        request.xml = self.create_cylinder_sdf(radius, height)
    ...
    def create_box_sdf(self, size_x, size_y, size_z):
        """Create SDF for a box"""
...
<sdf version="1.6">
  <model name="box">
    ...
    def create_cylinder_sdf(self, radius, height):
        """Create SDF for a cylinder"""
        return f'''<?xml version="1.0" ?>
<sdf version="1.6">
...
def main():
    ...
    try:
        # Spawn objects
        print("Spawning red box...")
        future1 = spawner.spawn_box("red_box", 1.0, 0.0, 0.25)
        rclpy.spin_until_future_complete(spawner, future1)
        
        print("Spawning green cylinder...")  
        future2 = spawner.spawn_cylinder("green_cylinder", -1.0, 0.0, 0.25)
        rclpy.spin_until_future_complete(spawner, future2)
        
        print("Objects spawned successfully!")
        
        # Wait for user input to delete
        input("Press Enter to delete objects...")
        
        # Delete objects
        future3 = spawner.delete_object("red_box")
        rclpy.spin_until_future_complete(spawner, future3)
        
        future4 = spawner.delete_object("green_cylinder")
        rclpy.spin_until_future_complete(spawner, future4)
