...
class AvoidConstruction(Node):
    ...
    def __init__(self):
        ...
        self.detect_intersection_type_sub = self.create_subscription(
            String,
            '/detect/intersection/type',
            self.intersection_type_callback,
            10
        )

        self.detect_level_type_sub = self.create_subscription(
            String,
            '/detect/level/type',
            self.level_type_callback,
            10
        )
        self.end_signal_sub = self.create_subscription(
            String,
            '/end_signal',
        ...
        self.tunnel_signal_pub = self.create_publisher(
            String,
            '/tunnel_signal',
            1
        )
        ...
        self.intersection_type_last_update_time = None 
        
        # Level crossing parameters
        self.level_type = None
        self.level_last_update_time = None  # Last time level signal was received
        self.level_signal_timeout = 2.0  # Timeout for level signal (seconds)
        self.level_active = False  # Whether level crossing control is active
        self.level_stop_delay_time = None  # Time when level stop was first detected
        self.level_stop_delay_duration = 1.0  # Continue for 1 second after level stop detection
        ...
        # States: 'NORMAL', 'AVOID_TURN', 'AVOID_STRAIGHT', 'RETURN_TURN', 'TRAFFIC_STOP', 'LEVEL_STOP'
        self.state = 'NORMAL'
    ...
    def odom_callback(self, msg):
        ...
        self.get_logger().info(f"x = {self.current_pos_x}")
        self.get_logger().info(f"y = {self.current_pos_y}")
    ...
    def intersection_type_callback(self, msg):
        self.intersection_type = msg.data
        self.intersection_type_last_update_time = self.get_clock().now()
        self.get_logger().info(f'intersection detected: {self.intersection_type}')

    def level_type_callback(self, msg):
        self.level_type = msg.data
        self.level_last_update_time = self.get_clock().now()
        self.level_active = True
        self.get_logger().info(f'level detected: {self.level_type}')
    ...
    def process_loop(self):
        ...
        # Check if level signal has expired
        self.check_level_signal_timeout()
        
        # Check level crossing conditions first (highest priority)
        if self.should_handle_level():
            self.process_level_state()
        # Then check traffic light conditions
        elif self.should_handle_traffic():
            self.process_traffic_state()
        # Finally handle normal states
        ...
        elif self.state == 'LEVEL_STOP':
            self.process_level_stop_state()
    ...
    def check_level_signal_timeout(self):
        """Check if level signal has timed out and deactivate if necessary"""
        if (self.level_active and 
            self.level_last_update_time is not None):
            
            current_time = self.get_clock().now()
            time_diff = (current_time - self.level_last_update_time).nanoseconds / 1e9
            
            if time_diff > self.level_signal_timeout:
                self.get_logger().info('Level signal timeout - deactivating level crossing control')
                self.level_active = False
                self.level_type = None
                if self.state == 'LEVEL_STOP':
                    self.state = 'NORMAL'
                    self.publish_active(False)

    def should_handle_level(self):
        """Check if we should handle level crossing based on recent signal"""
        # Level crossing has highest priority - always handle if active
        # Only skip if we don't have a signal
        if (not self.level_active or self.level_type is None):
            return False
        
        return True

    def should_handle_traffic(self):
        """Check if we should handle traffic light based on recent signal and camera detection"""
        # Traffic light has high priority, but lower than level crossing
        # Only skip if we don't have a signal or level is active
    ...
    def process_level_state(self):
        """Handle level crossing behavior based on type"""            
        if self.level_type == 'stop':
            # First time detecting level stop - start delay timer
            if self.level_stop_delay_time is None:
                self.level_stop_delay_time = self.get_clock().now()
                self.get_logger().info('Level crossing STOP detected - Starting 1 second delay before stopping')
            
            # Check if delay period has passed
            current_time = self.get_clock().now()
            time_diff = (current_time - self.level_stop_delay_time).nanoseconds / 1e9
            
            if time_diff < self.level_stop_delay_duration:
                # Still in delay period - continue moving
                twist = Twist()
                ...
                # Delay period over - stop
                self.get_logger().info('Level crossing delay completed - Stopping')
                self.state = 'LEVEL_STOP'
                twist = Twist()  # Zero velocity
                self.avoid_cmd_pub.publish(twist)
                self.publish_active(True)
                return
        
        elif self.level_type == 'go':
            # Reset level stop delay timer
            self.level_stop_delay_time = None
            # Continue when level crossing 'go' is detected
            self.get_logger().info('Level crossing GO detected - Continue')
            self.state = 'NORMAL'
            self.publish_active(False)
            self.process_normal_state()  # Execute normal driving logic
            return

    def process_level_stop_state(self):
        """Handle stopping at level crossing"""
        if self.level_type == 'go':
            self.get_logger().info('Level crossing GO detected - Resuming normal operation')
            self.state = 'NORMAL'
            self.publish_active(False)
        else:
            # Stay stopped for level crossing
            twist = Twist()
            self.avoid_cmd_pub.publish(twist)
            self.publish_active(True)

    def process_traffic_state(self):
    ...
    def process_normal_state(self):
        ...
        if (self.lane_state == 0 and -1.81 <= self.current_pos_x <= -1.71 and -0.5 <= self.current_pos_y <= 0.5 and not TUNNEL_FLAG):
            ...
            msg.data = "1"
            TUNNEL_FLAG = True
            self.tunnel_signal_pub.publish(msg)
            self.get_logger().info('Tunnel signal sent - waiting for navigation')
            return
    ...
    def visualization(self, points):
        ...
        # Display current state and signal info
        ...
        level_text = f'Level: {self.level_type if self.level_type else "None"}'
        level_active_text = f'LC Active: {self.level_active}'
        ...
        cv2.putText(img_vis, level_text, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(img_vis, level_active_text, (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(img_vis, distance_text, (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
