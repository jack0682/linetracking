...
class NavigationTrigger(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Navigation trigger node started. Waiting for robot to reach odom({self.trigger_x_odom}, {self.trigger_y_odom})')
        
    def odom_callback(self, msg):
        current_x = msg.pose.pose.position.x
        current_y = msg.pose.pose.position.y
        
        # Calculate distance to trigger point in odom frame
        distance = math.sqrt((current_x - self.trigger_x_odom)**2 + (current_y - self.trigger_y_odom)**2)
        
        # Check if robot has reached the trigger position
        if distance < self.position_threshold and not self.navigation_triggered:
            self.get_logger().info(f'Robot reached trigger position! Distance: {distance:.2f}m')
