...
class DetectTrafficLight(Node):
    ...
    def __init__(self):
        ...
        self.detect_traffic_type = self.create_publisher(String, '/detect/traffic/type', 1)
    ...
    def find_circle_of_traffic_light(self, mask, color):
        ...
        def is_valid_circle(x, y):
            ...
            circles = cv2.HoughCircles(
                ...
                param1=100,         # Canny 에지 검출 상위 임계값
                param2=8,         # 누적기 임계값 (작을수록 더 많은 원 검출)
                minRadius=8,       # 최소 반지름
                maxRadius=15       # 최대 반지름
            )
        ...
        for i in range(len(keypts)):
            ...
            if roi_x_start < self.point_x < roi_x_end and roi_y_start < self.point_y < roi_y_end :
                ...
                if is_valid_circle(self.point_x, self.point_y):
                    detect_result = True
                    self.get_logger().info(f'{color} light detected')
                    msg = String()
                    msg.data = f'{color}'
                    self.detect_traffic_type.publish(msg)
                    
                    # 디버깅용 원 그리기 (선택사항)
                    cv2.circle(self.cv_image, (self.point_x, self.point_y), 15, (255, 0, 255), 2)
                    cv2.imshow('Traffic Light Detection', self.cv_image)
                    cv2.waitKey(1)
                    break  # 첫 번째 유효한 원만 검출
                else:
                    detect_result = False
            else:
                detect_result = False
